...
public class ServiceManagementServiceImpl implements ServiceManagementService {
    ...
    public ServiceManagementServiceImpl(SalonServiceRepository salonServiceRepository) {
    ...
    public void init() {
        ...
        // Sync default services from services.txt to database
        try {
            List<SalonService> fileServices = getAllServices();
            for (SalonService fs : fileServices) {
                if (!salonServiceRepository.findByServiceId(fs.getServiceId()).isPresent()) {
                    salonServiceRepository.save(fs);
                }
            }
        } catch (Exception e) {
            System.err.println("Error syncing services from file to database: " + e.getMessage());
        }
    }
    ...
    public void addService(SalonService salonService) {
        ...
        // Sync to database
        try {
            salonServiceRepository.findByServiceId(salonService.getServiceId())
                    .ifPresent(existing -> salonService.setId(existing.getId()));
            salonServiceRepository.save(salonService);
        } catch (Exception e) {
            System.err.println("Database sync failed for addService: " + e.getMessage());
        }
    }
    ...
    public void updateService(SalonService updatedService) {
        ...
        // Sync to database
        try {
            salonServiceRepository.findByServiceId(updatedService.getServiceId())
                    .ifPresent(existing -> updatedService.setId(existing.getId()));
            salonServiceRepository.save(updatedService);
        } catch (Exception e) {
            System.err.println("Database sync failed for updateService: " + e.getMessage());
        }
    }
    ...
    public void deleteService(String serviceId) {
        ...
        // Sync to database
        try {
            salonServiceRepository.findByServiceId(serviceId)
                    .ifPresent(salonServiceRepository::delete);
        } catch (Exception e) {
            System.err.println("Database sync failed for deleteService: " + e.getMessage());
        }
    }
